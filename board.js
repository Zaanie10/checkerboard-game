...
var Board = function(size) {
    ...
    // Zaanie: Add array to track all checkers for rendering
    this.checkers = [];
    ...
    this.getAllCheckers = function(){
        // Zaanie: Return the list of all currently tracked checkers
        return this.checkers.slice(); // defensive copy
    }
	...
	this.add = function(checker, row, col){
		if (this.isEmptyLocation(row,col)){
            ...
            // Zaanie: Add checker to tracking list
	...
	this.remove = function(checker) {
        ...
        // Zaanie: Remove checker from tracking list
	...
	this.removeAt = function(row, col) {
		if (!this.square[row][col]){
            ...
            // Zaanie: Remove from checkers list
            var index = this.checkers.indexOf(this.square[row][col]);
            if (index !== -1) {
                this.checkers.splice(index, 1);
            }
	...
	this.clear = function() {
        ...
        // Zaanie: Also clear the tracking array
        this.checkers = [];
	}
